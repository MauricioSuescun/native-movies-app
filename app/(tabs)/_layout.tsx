...
const TabIcon = ({focused, icon, title}: {focused: boolean, icon: any, title: string}) => {
  ...
  return (
    <ImageBackground source={images.highlight} className="flex flex-row w-full flex-1 min-w-[112px] min-h-16 mt-4 items-center justify-center rounded-full overflow-hidden">
                <Image source={icon} tintColor={focused ? "#151312" : "#808080"} className="size-5"/>
                <Text className={`text-secondary font-semibold text-base ml-2 ${focused ? "text-primary" : "text-secondary"}`}>{title}</Text>
              </ImageBackground> 
  )
  }
  return (
